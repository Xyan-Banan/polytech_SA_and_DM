...
fun task2(price: Price, demProbs: List<DemandProbability>) {
    val orderDemand =
        mapOf(demProbs.map { it to mutableMapOf(demProbs.map { it.quantity.toFloat() } zip Array(demProbs.size) { 0f }) })

    println(demProbs.map { it.quantity }.joinToString("\t\t\t", prefix = "\t\t"))
    for ((order, demand) in orderDemand) {
        for (FHDemand in demand.keys) {
            demand[FHDemand] = with(price) {
                firstHalf * min(order.quantity.toFloat(), FHDemand) + secondHalf * max(
                    0f,
                    order.quantity - FHDemand
                ) - cost * order.quantity
            }
        }
...
//    normalizing
    ...
    for ((order, demand) in orderDemand) {
        ...
        println(
            "${order.quantity} \t${
                demand.values.map { round(it * 100) / 100 }.joinToString(" \t\t")
            } \t|\t ${ers[order.quantity]}"
        )
    }
